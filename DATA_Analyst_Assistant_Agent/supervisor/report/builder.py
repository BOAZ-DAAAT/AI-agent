...
def build_report(
    ...
    *,
    ...
    decision_lines = _analysis_decision_lines(state)
    decision_section = ["", "## 사용자 분석 결정", *decision_lines] if decision_lines else []
    return "\n".join(
        [
            ...
            *decision_section,
            "",
            "## Visuals",
            "- Visualization artifacts are not generated by a separate visualization agent.",
            "",
            "## Evidence",
            *evidence_lines,
            "",
...
def _analysis_decision_lines(state: OrchestrationState) -> list[str]:
    lines: list[str] = []
    for decision in state.analysis_review_decisions:
        approval_id = str(decision.get("approval_id") or "")
        review_request = decision.get("review_request") or {}
        followup = bool(review_request.get("requires_followup_analysis", True))
        selected_option = decision.get("selected_option")
        selection = decision.get("selection_response") or {}
        lines.append(f"### {approval_id}")
        if isinstance(selected_option, dict):
            lines.extend(
                [
                    f"- option ID: {selected_option.get('id', '')}",
                    f"- label: {selected_option.get('label', '')}",
                    f"- method: {selected_option.get('method', '')}",
                ]
            )
        else:
            free_text = str(selection.get("free_text") or "").replace("\r\n", "\n").replace("\r", "\n")
            lines.append("- 사용자 의견:")
            lines.extend(f"> {line}" for line in free_text.split("\n"))
        lines.append(f"- 후속 분석: {'필요' if followup else '불필요'}")
        lines.append("")
