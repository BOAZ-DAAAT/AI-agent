...
def distribution_node(state: EDAState) -> dict:
    ctx = get_context()
    if ctx.df is None:
        return {"distribution_result": "데이터가 로드되지 않았습니다.", "distribution_facts": [],
    ...
    return {"distribution_result": result, "distribution_facts": facts, "error_log": append_errors(state, err)}
