...
def relationship_node(state: EDAState) -> dict:
    ctx = get_context()
    if ctx.df is None:
        return {"relationship_result": "데이터가 로드되지 않았습니다.", "relationship_facts": [],
    ...
    return {"relationship_result": result, "relationship_facts": facts, "error_log": append_errors(state, err)}
