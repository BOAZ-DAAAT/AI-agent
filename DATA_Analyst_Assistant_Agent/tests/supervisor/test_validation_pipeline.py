...
def test_approval_is_decided_only_after_result_and_semantic_checks() -> None:
    ...
    result = AgentCompactResult(
        ...
        summary="?뱀씤 ?꾩슂",
        artifact_ids=["analysis_001"],
        approval=ApprovalRequirement(required=True, reason="寃?좉? ?꾩슂?⑸땲??"),
    )
...
def test_semantic_missing_evidence_without_reason_generates_limitation_message() -> None:
    ...
    record = updates["pending_validation"]
    expected_suffix = "analysis_table, segment_summary"
    assert record["checks"][-1]["findings"][0]["message"].endswith(expected_suffix)
    assert record["outcome"]["reason"].endswith(expected_suffix)
...
def test_semantic_warning_without_missing_evidence_is_accepted_with_limitations() -> None:
    ...
    record = updates["pending_validation"]
    assert record["checks"][-1]["passed"] is True
    assert record["checks"][-1]["findings"][0]["message"] == "?쇰? 湲곌컙 ?곗씠?곌? ?ъ냼?⑸땲??"
    assert record["outcome"]["disposition"] == "accept_with_limitations"
...
def test_semantic_info_with_invalid_flag_becomes_analysis_limitation() -> None:
    model = SemanticModel(
        {
            **_semantic_success("call_sql_agent"),
            "semantic_valid": False,
            "reason": "?묐떟???좏슚???쒖떆媛 紐⑥닚?⑸땲??",
        }
    )
    result = AgentCompactResult(
        agent="analysis_agent",
...
def test_semantic_error_becomes_analysis_limitation() -> None:
    model = SemanticModel(
        {
            **_semantic_success("call_eda_agent"),
            "severity": "error",
            "reason": "?꾩닔 遺꾩꽍???꾨씫?섏뿀?듬땲??",
...
def test_analysis_semantic_model_failure_retries_once_then_limits() -> None:
    ...
    outcome = updates["pending_validation"]["outcome"]
...
def test_semantic_create_plan_contract_violation_limits_analysis_candidate() -> None:
    ...
    assert model.calls == 2
    assert updates["pending_validation"]["outcome"]["disposition"] == "accept_with_limitations"
    assert updates["pending_validation"]["outcome"]["reason_code"] == "none"
