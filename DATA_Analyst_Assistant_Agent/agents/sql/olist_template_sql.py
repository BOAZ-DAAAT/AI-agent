...
SELECT sc.order_status,
       sc.order_count,
       ROUND(100.0 * sc.order_count / NULLIF(st.total_order_count, 0), 2) AS order_percentage
FROM status_counts AS sc
CROSS JOIN status_total AS st
GROUP BY sc.order_status, sc.order_count, st.total_order_count
ORDER BY sc.order_count DESC, sc.order_status
/*__LIMIT__*/;""",
